...
public class QuickHull {
    ...
    public static List<Vector2f> calcConvexHull(List<Vector2f> points) {
     ...
     static void findHull(List<Vector2f> subList, Vector2f v1, Vector2f v2, List<Vector2f> convexHull) {
     ...
     static double calcDistanceToLine(Vector2f v1, Vector2f v2, Vector2f point) {
        return (Math.abs((v2.x - v1.x) * (v1.y - point.y) - (v1.x - point.x) * (v2.y - v1.y)) /
                Math.sqrt((Math.pow((v2.x - v1.x), 2) + Math.pow( v2.y - v1.y , 2)))  );
    }

     static void aboveAndBelow(List<Vector2f> points, Vector2f leftMost, Vector2f rightMost, List<Vector2f> above, List<Vector2f> below) {
